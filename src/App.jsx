    ...
    <div className='bg-black'>
      ...
      <div className="relative z-10 h-dvh flex items-center pl-10 " id='home'>
        <div className="w-2/5 opacity-0 slide-fade-in animation-delay-1000 big-shoulders text-6xl md:text-8xl text-primary font-bold text-balance md:text-nowrap text-left ">
          ...
          <button className='cursor-pointer bg-primary dark:text-black text-white rounded-lg md:w-auto px-2 py-1 text-lg dm-sans md:mx-1 font-medium transition-all w-50 duration-300  hover:scale-105' onClick={() => {
            document.getElementById("services")?.scrollIntoView({ behavior: "smooth" });
          }}>
            Our Services
          </button>

          <button className=' cursor-pointer outline-2 dark:outline-[#E0F11F] outline-[#1F67F1] text-primary rounded-lg font-medium px-2 py-1 text-lg dm-sans w-50 md:w-auto md:mx-4 transition-all duration-300 hover:scale-105' onClick={() => {
        ...
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-20 hidden md:block">
          <a href="#about">
            <div className="w-12 h-12 rounded-full border-2 dark:border-[#E0F11F] border-[#1F67F1]  flex items-center justify-center cursor-pointer hover:bg-primary transition-all duration-300 group animate-pulse-scale">
              <svg
      ...
      <footer className="dark:bg-black text-white py-10">
